...
_ENUM_OPTIONS: dict[str, tuple[str, ...]] = {
    ...
    "heating_circuit_summer_winter_mode": ("off", "summer", "winter", "auto"),
    "heating_circuit_heat_cool_mode": ("heat", "cool", "auto"),
    ...
    "hot_water_operation_mode": ("off", "Off", "low", "high", "ownprogram", "eco"),
    ...
    "energy_management_status": ("notConnected", "connected", "active", "error"),
...
def _enum_options(resource: Resource, translation_key: str) -> list[str]:
    ...
    for candidate in (
        ...
        *resource.values,
        ...
        if isinstance(candidate, str) and candidate not in options:
