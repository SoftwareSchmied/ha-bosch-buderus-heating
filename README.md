...
> [!NOTE]
> **Deutschsprachige Anleitung:** Die bebilderte
> [Einrichtung für Einsteiger](docs/einrichtung.md) erklärt Installation und
...
- **Understandable:** German and English terminology is based on the heating
  ...
  `Heizkreis 1 – Betriebsart` that remain clear on larger installations.
...
[illustrated German guide](docs/einrichtung.md#das-repository-erscheint-noch-nicht-in-der-hacs-suche)
...
[illustrated German guide](docs/einrichtung.md#mit-singlekey-id-anmelden)
...
- `Heizkreis 2 – Raumsolltemperatur`
- `Warmwasser – Ist-Temperatur`
- `Wärmepumpe – Systemdruck`
- `Gesamt – Stromverbrauch`

The [entity and polling catalog](docs/entitaeten.md) documents every entity in
...
|---|---|
| `Gesamt – Stromverbrauch` | `totalConsumption → electricity`, otherwise `compressor + eheater` |
| `Gesamt – Stromverbrauch Wärmepumpe` | `totalConsumption → compressor` |
| `Gesamt – Stromverbrauch elektrischer Zuheizer` | `totalConsumption → eheater` |
| `Gesamt – Erzeugte Wärme` | `totalConsumption → outputProduced` |
| `Gesamt – Umweltenergie (berechnet)` | `outputProduced - compressor - eheater` |
| `Heizung – …` | corresponding values from `chConsumption` |
| `Warmwasser – …` | corresponding values from `dhwConsumption` |
| `Kühlung – …` | corresponding values from `coolingConsumption` |
...
[Reconfiguration and rediscovery](docs/neu-konfigurieren.md).
...
| Document | Contents |
|---|---|
| [Einrichtung für Einsteiger](docs/einrichtung.md) | Illustrated installation and SingleKey ID walkthrough |
| [Entitäten und Polling](docs/entitaeten.md) | Entity names, API resources, write access, and intervals |
| [Ressourcenkatalog](docs/ressourcenkatalog.md) | Observed PointT resources and terminology |
| [Neu konfigurieren](docs/neu-konfigurieren.md) | Gateway selection, rediscovery, and polling profiles |
| [Diagnosedaten](docs/diagnose.md) | Redacted diagnostics and request metrics |
| [Anonymisierte Inventur](docs/anonymisierte-inventur.md) | Value-free compatibility inventory for maintainers |
