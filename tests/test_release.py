...
def test_release_archive_is_reproducible_and_component_rooted(tmp_path: Path) -> None:
    first, checksum = build_release(tmp_path / "first", expected_version="0.5.1")
    second, _ = build_release(tmp_path / "second", expected_version="0.5.1")
...
def test_release_archive_imports_from_an_isolated_install(tmp_path: Path) -> None:
    archive, _ = build_release(tmp_path / "dist", expected_version="0.5.1")
